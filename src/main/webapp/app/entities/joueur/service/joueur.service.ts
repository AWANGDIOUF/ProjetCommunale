...
export class JoueurService {
  ...
  protected convertDateFromClient(joueur: IJoueur): IJoueur {
    return Object.assign({}, joueur, {
      dateNaisJoueur: joueur.dateNaisJoueur?.isValid() ? joueur.dateNaisJoueur.format(DATE_FORMAT) : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.dateNaisJoueur = res.body.dateNaisJoueur ? dayjs(res.body.dateNaisJoueur) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((joueur: IJoueur) => {
        joueur.dateNaisJoueur = joueur.dateNaisJoueur ? dayjs(joueur.dateNaisJoueur) : undefined;
      });
    }
    return res;
  }
