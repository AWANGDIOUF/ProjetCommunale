...
export class TypeVaccinComponent implements OnInit {
  ...
  trackId(index: number, item: ITypeVaccin): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: ITypeVaccin[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/type-vaccin'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
