...
export class VaccinationUpdateComponent implements OnInit {
  ...
  typeVaccinsCollection: ITypeVaccin[] = [];
  ...
  trackTypeVaccinById(_index: number, item: ITypeVaccin): number {
    return item.id!;
  }
  ...
  protected updateForm(vaccination: IVaccination): void {
    ...
    this.typeVaccinsCollection = this.typeVaccinService.addTypeVaccinToCollectionIfMissing(
      this.typeVaccinsCollection,
  ...
  protected loadRelationshipsOptions(): void {
    this.typeVaccinService
      .query({ filter: 'vaccination-is-null' })
      .pipe(map((res: HttpResponse<ITypeVaccin[]>) => res.body ?? []))
      .pipe(
        map((typeVaccins: ITypeVaccin[]) =>
          this.typeVaccinService.addTypeVaccinToCollectionIfMissing(typeVaccins, this.editForm.get('typeVaccin')!.value)
        )
      )
      .subscribe((typeVaccins: ITypeVaccin[]) => (this.typeVaccinsCollection = typeVaccins));
