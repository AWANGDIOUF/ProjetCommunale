...
export class VainqueurComponent implements OnInit {
  ...
  trackId(index: number, item: IVainqueur): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: IVainqueur[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/vainqueur'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
