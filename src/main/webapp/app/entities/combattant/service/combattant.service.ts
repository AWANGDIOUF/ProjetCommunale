...
export class CombattantService {
  ...
  protected convertDateFromClient(combattant: ICombattant): ICombattant {
    return Object.assign({}, combattant, {
      dateNaisCombattant: combattant.dateNaisCombattant?.isValid() ? combattant.dateNaisCombattant.format(DATE_FORMAT) : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.dateNaisCombattant = res.body.dateNaisCombattant ? dayjs(res.body.dateNaisCombattant) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((combattant: ICombattant) => {
        combattant.dateNaisCombattant = combattant.dateNaisCombattant ? dayjs(combattant.dateNaisCombattant) : undefined;
      });
    }
    return res;
  }
