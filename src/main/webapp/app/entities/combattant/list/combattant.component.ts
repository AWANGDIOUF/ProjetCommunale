...
export class CombattantComponent implements OnInit {
  ...
  trackId(index: number, item: ICombattant): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: ICombattant[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/combattant'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
