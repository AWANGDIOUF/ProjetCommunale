...
export class DonSangComponent implements OnInit {
  ...
  trackId(index: number, item: IDonSang): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: IDonSang[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/don-sang'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
