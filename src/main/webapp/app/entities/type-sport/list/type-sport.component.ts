...
export class TypeSportComponent implements OnInit {
  ...
  trackId(index: number, item: ITypeSport): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: ITypeSport[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/type-sport'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
