...
export class ArchiveSportComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.archiveSportService
      ...
      .subscribe(
        (res: HttpResponse<IArchiveSport[]>) => {
  ...
  trackId(index: number, item: IArchiveSport): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: IArchiveSport[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/archive-sport'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
