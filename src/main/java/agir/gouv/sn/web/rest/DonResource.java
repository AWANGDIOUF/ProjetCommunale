...
import agir.gouv.sn.repository.DonRepository;
...
/**
 * REST controller for managing {@link agir.gouv.sn.domain.Don}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class DonResource {
     ...
     */
    @GetMapping("/dons")
    public List<Don> getAllDons(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get all Dons");
        return donRepository.findAllWithEagerRelationships();
    }
    ...
    public ResponseEntity<Don> getDon(@PathVariable Long id) {
        ...
        Optional<Don> don = donRepository.findOneWithEagerRelationships(id);
