...
import agir.gouv.sn.repository.QuartierRepository;
...
/**
 * REST controller for managing {@link agir.gouv.sn.domain.Quartier}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class QuartierResource {
    ...
    public List<Quartier> getAllQuartiers(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get all Quartiers");
        return quartierRepository.findAllWithEagerRelationships();
    }
    ...
    public ResponseEntity<Quartier> getQuartier(@PathVariable Long id) {
        ...
        Optional<Quartier> quartier = quartierRepository.findOneWithEagerRelationships(id);
