...
import agir.gouv.sn.repository.VainqueurRepository;
...
/**
 * REST controller for managing {@link agir.gouv.sn.domain.Vainqueur}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class VainqueurResource {
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/vainqueurs/{id}", consumes = { "application/json", "application/merge-patch+json" })
    ...
    public List<Vainqueur> getAllVainqueurs(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get all Vainqueurs");
        return vainqueurRepository.findAllWithEagerRelationships();
    }
    ...
    public ResponseEntity<Vainqueur> getVainqueur(@PathVariable Long id) {
        ...
        Optional<Vainqueur> vainqueur = vainqueurRepository.findOneWithEagerRelationships(id);
