...
/**
 * REST controller for managing {@link agir.gouv.sn.domain.Partenaires}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class PartenairesResource {
    ...
    public ResponseEntity<Partenaires> partialUpdatePartenaires(
        ...
        Optional<Partenaires> result = partenairesRepository
            ...
            .map(existingPartenaires -> {
                if (partenaires.getNomPartenaire() != null) {
                    existingPartenaires.setNomPartenaire(partenaires.getNomPartenaire());
                }
                if (partenaires.getEmailPartenaire() != null) {
                    existingPartenaires.setEmailPartenaire(partenaires.getEmailPartenaire());
                }
                if (partenaires.getTelPartenaire() != null) {
                    existingPartenaires.setTelPartenaire(partenaires.getTelPartenaire());
                }
    ...
    public List<Partenaires> getAllPartenaires(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get all Partenaires");
        return partenairesRepository.findAllWithEagerRelationships();
    }
    ...
    public ResponseEntity<Partenaires> getPartenaires(@PathVariable Long id) {
        ...
        Optional<Partenaires> partenaires = partenairesRepository.findOneWithEagerRelationships(id);
