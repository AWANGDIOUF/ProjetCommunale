...
public interface InscriptionRepository extends JpaRepository<Inscription, Long> {
    ...
    @Query(
        value = "select distinct inscription from Inscription inscription left join fetch inscription.evenement",
    ...
    @Query("select distinct inscription from Inscription inscription left join fetch inscription.evenement")
    List<Inscription> findAllWithToOneRelationships();

    @Query("select inscription from Inscription inscription left join fetch inscription.evenement where inscription.id =:id")
