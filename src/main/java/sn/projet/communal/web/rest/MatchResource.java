...
/**
 * REST controller for managing {@link sn.projet.communal.domain.Match}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class MatchResource {
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/matches/{id}", consumes = "application/merge-patch+json")
            ...
            .map(
                existingMatch -> {
                    ...
                    if (match.getLieu() != null) {
                        existingMatch.setLieu(match.getLieu());
                    }
    ...
    public ResponseEntity<List<Match>> getAllMatches(
        ...
        Page<Match> page;
        if (eagerload) {
            page = matchRepository.findAllWithEagerRelationships(pageable);
        } else {
