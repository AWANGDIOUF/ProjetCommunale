...
import sn.projet.communal.repository.EquipeRepository;
...
/**
 * REST controller for managing {@link sn.projet.communal.domain.Equipe}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class EquipeResource {
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/equipes/{id}", consumes = "application/merge-patch+json")
            ...
            .map(
                existingEquipe -> {
                    ...
                    if (equipe.getDateCreation() != null) {
                        existingEquipe.setDateCreation(equipe.getDateCreation());
                    }
                    if (equipe.getLogo() != null) {
                        existingEquipe.setLogo(equipe.getLogo());
                    }
                    if (equipe.getLogoContentType() != null) {
                        existingEquipe.setLogoContentType(equipe.getLogoContentType());
                    }
