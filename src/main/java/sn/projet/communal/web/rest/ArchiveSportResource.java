...
import sn.projet.communal.repository.ArchiveSportRepository;
...
/**
 * REST controller for managing {@link sn.projet.communal.domain.ArchiveSport}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class ArchiveSportResource {
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/archive-sports/{id}", consumes = "application/merge-patch+json")
    ...
    public ResponseEntity<List<ArchiveSport>> getAllArchiveSports(
        Pageable pageable,
        @RequestParam(required = false, defaultValue = "false") boolean eagerload
    ) {
        log.debug("REST request to get a page of ArchiveSports");
        Page<ArchiveSport> page;
        if (eagerload) {
            page = archiveSportRepository.findAllWithEagerRelationships(pageable);
        } else {
            page = archiveSportRepository.findAll(pageable);
        }
