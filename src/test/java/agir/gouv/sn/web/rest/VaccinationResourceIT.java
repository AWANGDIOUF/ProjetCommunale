...
import agir.gouv.sn.repository.VaccinationRepository;
...
class VaccinationResourceIT {
    ...
    @Mock
    private VaccinationRepository vaccinationRepositoryMock;
    ...
    void getAllVaccinationsWithEagerRelationshipsIsEnabled() throws Exception {
        when(vaccinationRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restVaccinationMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(vaccinationRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllVaccinationsWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(vaccinationRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
