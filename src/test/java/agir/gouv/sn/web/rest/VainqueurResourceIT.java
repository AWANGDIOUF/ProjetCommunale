...
class VainqueurResourceIT {
    ...
    @Mock
    private VainqueurRepository vainqueurRepositoryMock;
    ...
    void getAllVainqueursWithEagerRelationshipsIsEnabled() throws Exception {
        when(vainqueurRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restVainqueurMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(vainqueurRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllVainqueursWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(vainqueurRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
