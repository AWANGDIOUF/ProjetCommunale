...
class DonResourceIT {
    ...
    void getAllDonsWithEagerRelationshipsIsEnabled() throws Exception {
        when(donRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restDonMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(donRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }
